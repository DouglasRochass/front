...
class DataTable {
  constructor(options = {}) {
    ...
    this.currentPage = 1;
    this.searchTerm = '';
    this.idField = options.idField || 'id'; // Campo que identifica cada linha
  }
  ...
  renderRow(row) {
    ...
    if (this.onEdit || this.onDelete || this.onView) {
      ...
      if (this.onView) {
        html += `<button class="btn-icon btn-view" title="Visualizar" data-id="${rowId}">👁</button>`;
      }
      
      if (this.onEdit) {
        html += `<button class="btn-icon btn-edit" title="Editar" data-id="${rowId}">✎</button>`;
      }
      
      if (this.onDelete) {
        html += `<button class="btn-icon btn-delete" title="Deletar" data-id="${rowId}">🗑</button>`;
      }
  ...
  filterData() {
    ...
    return this.data.filter(row => {
      return JSON.stringify(row).toLowerCase().includes(this.searchTerm);
    });
  }
